...
import { aiChat, aiChatStream, getProvidersStatus } from "../api";
...
export default function ChatPage() {
  const [providers, setProviders] = useState({ local: false, together: false, openrouter: false, deepseek: false, emergent: false });
  ...
  async function refreshProviders() {
    try { const s = await getProvidersStatus(); setProviders(s); } catch (e) { console.error(e); }
  }
  useEffect(() => { refreshProviders(); }, []);

  const send = async (stream = false) => {
    if (!prompt.trim() && !imageUrl.trim()) { toast.error("Enter a prompt or image URL"); return; }
    const messages = [{ role: "user", content: prompt.trim() }];
    const usedProvider = provider === "auto" ? null : provider;
    setAnswer(""); setAnswerMeta({ provider: "-", model: "-" }); setLoading(!stream); setStreaming(stream);
    try {
      if (stream) {
        let accum = "";
        for await (const chunk of aiChatStream({ messages, model, temperature, max_tokens: maxTokens, provider: usedProvider, image_url: imageUrl || null })) {
          accum += chunk; setAnswer(accum);
        }
        setStreaming(false);
      } else {
        const res = await aiChat({ messages, model, temperature, max_tokens: maxTokens, provider: usedProvider, image_url: imageUrl || null });
        setAnswer(res.content); setAnswerMeta({ provider: res.provider, model: res.model }); setLoading(false);
      }
    } catch (e) {
      console.error(e); toast.error(e?.message || "Chat failed"); setLoading(false); setStreaming(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#0f0f10] text-white">
      <div className="max-w-5xl mx-auto px-4 py-10">
        ...
        <p className="text-gray-400 mt-1">Primary: Local H100; Fallback: OpenRouter → DeepSeek → Together → Emergent</p>

        <div className="mt-4 flex gap-2 items-center flex-wrap">
          <ProviderPill name="Local" enabled={providers.local} />
          <ProviderPill name="OpenRouter" enabled={providers.openrouter} />
          <ProviderPill name="DeepSeek" enabled={providers.deepseek} />
          <ProviderPill name="Together" enabled={providers.together} />
          <ProviderPill name="Emergent" enabled={providers.emergent} />
          <button className="ml-2 text-xs underline text-emerald-400" onClick={refreshProviders}>refresh</button>
        </div>

        <form onSubmit={(e)=>{e.preventDefault(); send(false);}} className="mt-6 space-y-4 bg-[#151517] rounded-lg p-5 border border-gray-800">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <select className="w-full rounded border border-gray-700 bg-transparent px-3 py-2" value={provider} onChange={(e) => setProvider(e.target.value)}>
                ...
                <option className="text-black" value="deepseek">DeepSeek</option>
                <option className="text-black" value="together">Together</option>
                <option className="text-black" value="emergent">Emergent</option>
              </select>
            </div>
            <div>
              <label className="block text-sm text-gray-400 mb-1">Model</label>
              <input className="w-full rounded border border-gray-700 bg-transparent px-3 py-2" value={model} onChange={(e) => setModel(e.target.value)} placeholder="e.g. gpt-4o-mini or deepseek-chat" />
            </div>
          ...
          <div className="flex gap-3">
            <button type="submit" disabled={loading || streaming} className="px-4 py-2 rounded bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50">{loading ? "Sending..." : "Send"}</button>
            <button type="button" onClick={()=>send(true)} disabled={loading || streaming} className="px-4 py-2 rounded bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50">{streaming ? "Streaming..." : "Stream"}</button>
          </div>
        </form>
